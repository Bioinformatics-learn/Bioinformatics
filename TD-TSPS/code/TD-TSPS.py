...
# get the read count for each location
...
# return the densest value in RD
def modeRD(RD):
...
# CBS segment  to be in the same directory as CBS_data.R
def Read_seg_file():
    """
...
# filter eligible data from the list
...
# SR-PEM collaborative optimization
    ...
    for i in range(len(SVstart)):
        ...
        for r in bf.fetch(chrname, SVstart[i] - maxbin * binSize, SVend[i] + maxbin * binSize):
            if r.cigarstring != str1 and r.cigarstring != None and r.tlen != 0:
                not100Mperrange[i].append(
                    [r.reference_name, r.pos, r.cigarstring, r.pnext, r.tlen, SVlen[i], r.flag & 64, r.flag & 128,
                     r.flag])
...
# tandem duplication by MCD

clf = MCD(support_fraction=0.3)
clf.fit(RDMQ_scaler)
labels = clf.labels_
...
for i in range(len(SVStart)):
    output.write(
        str(chrname) + '\t' + str(SVStart[i]) + '\t' + str(SVEnd[i]) + '\t' + str(
            SVEnd[i] - SVStart[i] + 1) + '\t' + str(number) + '\t' + '\n')
